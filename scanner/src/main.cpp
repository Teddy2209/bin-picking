...
//Dựng cửa sổ 3D để hiển thị đám mây điểm vừa quét được.
...
public:
	...
	void ScannerConnect()
	{
		...
		{
			SetSaveScanImages(true);
			SetSave3DOutput(true);
			SetCamExposureTime();
			SetCamTrigger();
			SetProjectionDelay();
	...
	{   // Minumum ExposureTime = 50 microsecond
		m_pScanCanner->SetExposureTime(0, 1000);
		printf("Current Exposure Cam 0: %f\n", m_pScanCanner->GetExposureTime(0)); 
		m_pScanCanner->SetExposureTime(1, 1000);
...
public:
	...
	virtual void ISCAPI2P_ScanFinish(int nResult) 
	{
		...
		if (nVisualizeMode == 0)
		{
			...
			cv::imshow("L", pImgL);
			cv::imwrite("/home/apicoo-ai/pmg/Scan3d_API_pmg/irovi3D_3D_scanner_api/output_data/image2d.BMP", pImgL);
			cv::waitKey(50);
	...
	virtual void ISCAPI2P_ScanFinish_save(int nResult) 
	{
		...
		if (nVisualizeMode == 0)
		{
			...
			// Save images
			cv::imwrite("left.png", pImgL);
			cv::imwrite("right.png", pImgR);
			...
			cv::imwrite("left_corners.png", imgColor);

			printf("Saved left_corners.png\n");
			// -----------------------------
			// Save point cloud (PLY)
			// -----------------------------
			std::ofstream ply_file("pointcloud.ply");
			ply_file << "ply\nformat ascii 1.0\n";
			...
			// Save UV mapping
			// -----------------------------
			std::ofstream uv_file("uv_cam1.txt");
			for (int i = 0; i < nPoints; i++) {
				int u = pUV[i * 2];
				int v = pUV[i * 2 + 1];
				uv_file << u << " " << v << "\n";
			...
			cv::FileStorage fs("disparity.yml", cv::FileStorage::WRITE);
...
void PressEnterToExit(void)
{
    ...
    while(c == 0)
    {
		...
		{
			/* do scan */
			pScannerCoreTestPtr->SetSave3DOutput(true);
